...
class ForgotPassword extends MY_Controller {
    ...
    public function forgotP()
    {
        ...
        $tanggal = date("Y-m-d", mktime(date('m'), date("d"), date('Y')));
            ...
            if($cekAkun > 0) {
                if (!empty($cekId)) {
                    flashMessage('warning', 'Akun Sudah Terdaftar, Silahkan Tunggu verivikasi admin 1x24jam');
                    ...
                    $anggota['status_pemulihan'] = 0;
                        ...
                        flashMessage('success', 'Berhasil! .. Silahkan Tunggu verivikasi admin 1x24jam');
                        redirect('login/forgotPassword');
                } else {
